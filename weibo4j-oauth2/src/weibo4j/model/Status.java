...
public class Status extends WeiboResponse implements java.io.Serializable {
	...
	private int mlevel;
	private List<String>   pic_urls;                              //图片组
	...
	private void constructJson(JSONObject json) throws WeiboException {
		try {
			...
			JSONArray pics = null;
			String pic_urlString;
			pic_urls=new ArrayList<String>();
			if(!json.isNull("pic_urls")){
				
				pics=json.getJSONArray("pic_urls");
				
				int size=pics.length();
				
				for(int i=0;i<size;i++){
					
		        	JSONObject picJsonObject=pics.getJSONObject(i);
				
				    pic_urlString=picJsonObject.getString("thumbnail_pic");
				    
				    pic_urls.add(pic_urlString);
